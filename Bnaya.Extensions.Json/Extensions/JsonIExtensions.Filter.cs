...
#pragma warning disable S3241 // Methods should not return values that are never used
        ...
        IWriteCommand? writeCommand = null)
    {
            ...
            using (var cmd1 = CreateArryWriter(writer, breadcrumbs.State, cmd))
            {
                ...
                foreach (var val in elements)
                {
                    using (var spine = breadcrumbs.Add($"[{i++}]"))
                    {
                        ...
                        {
                            var matchHook = mark == TraverseAction.TakeOrReplace ? onMatch : null;
                            using (var cmd2 = CreateElementWriter(val, matchHook, writer, breadcrumbs.State, cmd1))
                            {
                                cmd2.Run();
                            }
                        }
        ...
        else
        {
            ...
            {
                var matchHook = mark == TraverseAction.TakeOrReplace ? onMatch : null;
                using (var cmd1 = CreateElementWriter(element, matchHook, writer, breadcrumbs.State, cmd))
                {
                    cmd1.Run();
                }
            }
